...
class PGD:
    ...
    def generate_aux(self, model, x, y=None, targets=None, device=torch.device("cpu")):
        model.to(device)
        model.eval()
        x_adv = []
        for i in range(0, x.size(0), self.batch_size):
            ...
            for j in range(self.max_iter):
                if j == 0:
                    x_adv_batch = self.attack_aux(model, x_batch, y_batch, targets=targets)
                else:
                    x_adv_batch = self.attack_aux(model, x_batch, y_batch, x_adv_batch, targets=targets)
            x_adv.append(x_adv_batch)
        return torch.cat(x_adv, dim=0).cpu()
...
class PGD_M:
    ...
    def attack(self, model, x, y, x_adv=None, targets=None):
        ...
        x_adv = (x_adv.data + pert).clamp(0.0, 1.0)
        pert = (x_adv - x).clamp(-self.eps, self.eps)
        return x + pert
